...
- 批量评测(`python scripts/batch_eval.py`，30 轮随机注入，报告见 `data/processed/batch_eval_report.json`)：
  ...
  |---|---|
  | 预警检出率（首达断面） | **20%**(6/30) |
  | 预警检出率（任一受污染断面） | **30%**(9/30) |
  | 上游候选召回率 | **87%**(26/30) |
  | Top-1 / Top-3 命中率 | **73%** / **87%** |
  | 最终锁定命中率 | **73%**(22/30) |
  | MRR | **0.789** |
  | 传播时间误差(均值/最大) | **0.04h / 0.10h** |
  | 分类型检出(首达/任一) | sudden 60%/60% · periodic 0%/30% · gradual 0%/0% |
  | 分类型 Top-1 | sudden 80% / periodic 60% / gradual 80% |

  > 检出率两个口径：严格口径要求**真实首达断面**（溯源价值最高的那个）被检出；
  > 宽松口径只要求**任一受污染断面**被检出。周期型事件常在下游断面报出而首达断面未报，
  > 故两个口径分别是 0% 与 30%。
  >
  > 该指标历史上写过 100%，是三处口径问题叠加的结果：把"扫到任何告警"当命中、
  > 注入时点常落在扫描窗口外、以及监测 Agent 在量化噪声上误报（已由
  > `engine.anomaly.significant` 门槛修复）。逐轮污染累积也已修正（每轮从 SQLite 基线恢复）。
  > **周期型与渐变型漏检是当前最大短板**，方向是季节基线检测 + 更长基线窗口。
