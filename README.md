...
AQ_MONITOR_METHOD=cusum   # 检测方法:cusum|ewma|threesigma|seasonal(默认 cusum)
```
...
- 批量评测(`python scripts/batch_eval.py`，30 轮随机注入，报告见 `data/processed/batch_eval_report.json`)：
  ...
  | 指标 | 结果 |
  |---|---|
  | 预警检出率（**归因**，扣常驻告警） | **30%**(9/30) |
  | 预警检出率（首达断面，原始） | 67%(20/30) |
  | 预警检出率（任一受污染断面，原始） | 97%(29/30) |
  | 上游候选召回率 | **87%**(26/30) |
  | Top-1 / Top-3 命中率 | **73%** / **87%** |
  | 最终锁定命中率 | **73%**(22/30) |
  | MRR | **0.789** |
  | 传播时间误差(均值/最大) | **0.04h / 0.10h** |
  | 分类型归因检出 | sudden 70% · periodic 20% · gradual 0% |
  | 分类型 Top-1 | sudden 80% / periodic 60% / gradual 80% |

  > **检出率必须看归因口径**。干净数据上监测 Agent 本来就会常驻报出 8 个(断面,指标)
  > 的告警——24h 窗口的漂移检测无法区分"日内昼夜波动"与"污染"（实测干净序列的
  > 日内极差就是均值的 40%）。这 8 个常驻告警覆盖多个断面，会让"首达断面被检出"
  > 与"任一断面被检出"两个原始口径虚高（67% / 97%）；扣掉它们才是本次注入真正
  > 带来的检出，即 **30%**。
  >
  > 该指标历史上写过 100%，是口径问题层层叠加的结果：把"扫到任何告警"当命中、
  > 注入时点落在扫描窗口外、监测 Agent 在量化噪声上误报、逐轮污染累积、以及
  > 类型与严重度在评测里被绑死（突发恒 low / 周期恒 medium / 渐变恒 high，
  > 使分类型数字实际是组合数）。以上均已修正。
  >
  > **改用季节基线方法可把归因检出提到 73%**（sudden 10/10、periodic 7/10、
  > gradual 5/10，常驻告警 9 个）：`AQ_MONITOR_METHOD=seasonal`。默认仍是 cusum。
  > 进一步提升的关键是让报警门槛"认得昼夜规律"（同刻基线 + 去趋势），
  > 这也是当前最大的短板。
